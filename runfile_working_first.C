	 			...
	 			for (int a=0; a<24; ++a)
	 			    {
						...
						chi = 0;
						NDF = 0;
						for (int z=5; z<26; z++) {chi += pow( ( grBF->Eval(histo_LED->GetXaxis()->GetBinCenter(z)) - histo_LED->GetBinContent(z) )/histo_LED->GetXaxis()->GetBinError(z) , 2 );	if(histo_LED->GetBinContent(z)>0) {++NDF;} }
