	 			...
	 			for (int a=0; a<24; ++a)
	 			    {
						...
						if ((fit.chi2r <= 3) && (fit.fit_status == 0)  && (gainerror/Gfit<10) )	{ANGLES[count] = 15*a;  PMT_DATA[2*p-2][count] = Gfit;	PMT_DATA[2*p-1][count] = gainerror;   ++count;	STATUS = "Yes";}
						else {STATUS = "No"; ++REM;}
						...
						if ((fit.chi2r <= 3) && (fit.fit_status == 0) && (gainerror/Gfit<10) )
