				...
				Int_t count = 0;
	 			...
	 			for (int a=0; a<24; ++a)
					...
					if(SPEM == 1)
					{
						...
						if ((p==1)&&(a==1)) {TFile* f1 = new TFile("pos_1_15.root");	TGraph* graph1 = (TGraph*)f1->Get("grBF");	 graph1->SetLineColor(kRed);	graph1->Draw("");	c1->Update(); c1->WaitPrimitive(); c1->Print(PdfName_mid ,"pdf");}
