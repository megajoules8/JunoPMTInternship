	 ...
	 Int_t REM = 0;
		...
		KST->GetYaxis()->SetTitle("Counts");
	 	...
	 	{	
	 			...
	 			gaindata <<"angle Mu Mu_err w w_err alpha alpha_err lambda lambda_err Theta Theta_err sig_reduced sig_reduced_err Gain Gain_err chi_sq fit.status is Point Good?  KS"<< endl;
	 			...
	 			for (int a=0; a<24; ++a)
	 			    {
						...
						if ((fit.chi2r <= 3) && (fit.fit_status == 0) && (D<10)  )	{ANGLES[count] = 15*a;  PMT_DATA[2*p-2][count] = Gfit;	PMT_DATA[2*p-1][count] = gainerror;   ++count;	STATUS = "Yes";}
						else {STATUS = "No"; ++REM;}
